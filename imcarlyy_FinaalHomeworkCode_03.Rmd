...
```{r echo=TRUE}
require(readr)
file_path <- "C:/Users/Carly Rojas/OneDrive - Universidad San Francisco de Quito/Documentos/A_MS IN BIOLOGY/SCHMITT_588 Project Design and Statistics/zombies.csv" # manually specify your file path
survivors <- read_csv(file_path, col_names = TRUE)# for comma-separated files
```
...
```{r echo=TRUE}
height_scatterplot <- ggplot(data = survivors, aes(x = age, y = height, color = factor(gender))) + 
  xlab("Age") + 
  ylab("Height") + 
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +  # Adding the regression line suggested.
...
weight_scatterplot <- ggplot(data = survivors, aes(x = age, y = weight, color = factor(gender))) + 
  xlab("Age") + 
  ylab("Weight") + 
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +  # Adding the regression line suggested.
...
grid.arrange(height_scatterplot, weight_scatterplot, ncol = 2, top = "Scatterplots of Height and Weight in Relation to Age with Regression Lines")
 #It was very stasfying to learn about gridExtra because it made visualizing my plots much easier! 
```
...
#My answer: This one seems to be a normal distribution.
```
...
print(c(
  ...
  Weight = mean_weight, 
  Age = mean_age, 
  Zombies_Killed = mean_zombies_killed, 
  Years_of_Education = mean_education
))
...
print(c(
  ...
  Weight = sd_weight, 
  Age = sd_age, 
  Zombies_Killed = sd_zombies_killed, 
  Years_of_Education = sd_education
))
